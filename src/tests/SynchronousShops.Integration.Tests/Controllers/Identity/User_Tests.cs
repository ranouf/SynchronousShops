...
namespace SynchronousShops.Integration.Tests.Controllers.Identity
{
    ...
    public class Users_Tests : BaseTest
    {
        ...
        public async Task Should_Get_Two_Users_Order_By_LastName_As_Administrator()
        {
            ...
            var dto = await response.ConvertToAsync<List<UserDto>>(Output);
        ...
        public async Task Should_Get_By_User_Id_As_Administrator()
        {
            ...
            var administrator = usersDto.First(u => u.RoleName == Domains.Core.Constants.Roles.Administrator);
        ...
        public async Task Should_Not_Lock_Myself_As_Administrator()
        {
            ...
            // Not Lock
            response = await Client.PutByIdAsync(
                Libraries.Constants.Api.V1.User.Lock,
                Output,
                dto.CurrentUser
            );
            Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
        ...
        public async Task Should_Lock_And_Unlock_User_As_Administrator()
        {
            ...
            // Unlock User
            response = await Client.PutByIdAsync(
                Libraries.Constants.Api.V1.User.Unlock,
                Output,
                dto.CurrentUser
            );
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        ...
        [Fact]
        public async Task Should_Invite_ReInvite_Confirm_Login_And_Delete_As_Administrator()
        {
            // As Admin
            await Client.AuthenticateAsAdministratorAsync(Output);
            ...
            response = await Client.PutAsync(
                Libraries.Constants.Api.V1.Authentication.ConfirmInvitationEmail,
                Output,
